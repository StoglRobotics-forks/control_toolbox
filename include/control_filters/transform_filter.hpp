...
#ifndef CONTROL_FILTERS__TRANSFORM_FILTER_HPP_
#define CONTROL_FILTERS__TRANSFORM_FILTER_HPP_
...
#include "transform_filter_parameters.hpp"

namespace control_filters
{

template <typename T>
class TransformFilter : public filters::FilterBase<T>
{
public:
  /** \brief Constructor */
  TransformFilter();
...
protected:
  ...
  {
    if (parameters_.target_frame_id.empty())
    {
      RCLCPP_ERROR((*logger_), "Target frame id is empty");
      return false;
    }

    if (parameters_.sensor_frame_id.empty())
    {
      RCLCPP_ERROR((*logger_), "Sensor frame id is empty");
      return false;
    }

    target_frame_id_ = parameters_.target_frame_id;
    sensor_frame_id_ = parameters_.sensor_frame_id;
    return true;
  };
...
private:
  ...
  std::shared_ptr<transform_filter::ParamListener> parameter_handler_;
  transform_filter::Params parameters_;
...
template <typename T>
TransformFilter<T>::TransformFilter()
{
}
...
template <typename T>
bool TransformFilter<T>::configure()
{
  ...
  if (!parameter_handler_)
  {
    try
    {
      parameter_handler_ = std::make_shared<transform_filter::ParamListener>(
...
#endif  // CONTROL_FILTERS__TRANSFORM_FILTER_HPP_
